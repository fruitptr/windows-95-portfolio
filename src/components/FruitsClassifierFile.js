...
function FruitsClassifierFile({ content }) {
  ...
  return (
    <Frame bg="white" boxShadow="in" className="frame-container-project">
      ...
      <p>{content[1]["content"][5]}</p>
      <p id="bold-p">{content[1]["content"][6]}<a href={content[3]["content"][0]}><span>{content[3]["content"][0]}</span></a></p>
      <p id="bold-p">{content[1]["content"][7]}<a href={content[3]["content"][1]}><span>{content[3]["content"][1]}</span></a></p>
    </Frame>
  );
}

export default FruitsClassifierFile;